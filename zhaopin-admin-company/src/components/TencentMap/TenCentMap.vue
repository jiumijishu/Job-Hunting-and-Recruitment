<template>
  <div :style="{width}">
    ...
    <!-- 周边位置 -->
    <div class="bounds" :style="{height: boundsHeight}" ref="bounds">
      <div class="list" v-for="(item, index) in BoundsPois" :key="index" @click="selectCity(index)">
        <div>
          <p class="name">{{item.name}}</p>
          <span class="address">{{item.address}}</span>
        </div>
        <span class="yes" :class="{on: selectPosition == index}"></span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      latLng: [this.lat?this.lat:null, this.lng?this.lng:null], //默认位置，之后会自动定位到当前位置。
  ...
  methods: {
    ...
    init() {
      ...
      if(this.lat !== null && this.lng !== null){
        this.getLocation2(this.lat,this.lng);
      }else{
        this.getLocation();
      }
      //绑定地图点击事件
    ...
    //获取当前位置
    getLocation2(lat,lng) {
      let cs = new qq.maps.CityService({
        map: this.map,
        complete: results => {
          results.detail.latLng.lat= Number(lat)
          results.detail.latLng.lng= Number(lng)
          this.latLng = [results.detail.latLng.lat, results.detail.latLng.lng];
          this.latLng = [results.detail.latLng.lat, results.detail.latLng.lng];
          //设置地图中心位置为当前定位。
          this.map.setCenter(results.detail.latLng);
