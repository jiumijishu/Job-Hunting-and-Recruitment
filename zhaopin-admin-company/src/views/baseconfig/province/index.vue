    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['baseconfig:province:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['baseconfig:province:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['baseconfig:province:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['baseconfig:province:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" border :data="provinceList" @selection-change="handleSelectionChange">
...
<script>
import { listProvince, getProvince, delProvince, addProvince, updateProvince } from "@/api/baseconfig/province";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.provinceId)
    ...
    handleUpdate(row) {
      this.reset();
      const provinceId = row.provinceId || this.ids
      getProvince(provinceId).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const provinceIds = row.provinceId || this.ids;
      this.$modal.confirm('是否确认删除省份信息编号为"' + provinceIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('baseconfig/province/export', {
        ...this.queryParams
