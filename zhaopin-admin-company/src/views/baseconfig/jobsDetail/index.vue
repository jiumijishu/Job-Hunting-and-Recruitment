        ...
        <div class="jobsData">
            <div>
              <el-row class="jobInfo">
                <el-col :span="18"><div class="">  <span>{{jobsData.jobName}}</span> ·  <span>{{jobsData.typeName}}</span></div></el-col>
                <el-col :span="6"><div class="grid-content">{{jobsData.minSalary}}~{{jobsData.maxSalary}}</div></el-col>
              </el-row>
            </div>
            ...
            <div class="titles">工作经验</div>
            <div style="text-indent:6px;">{{jobsData.workExperience}}</div>

            <div class="titles" >职位性质</div>
            <div v-html="jobsData.jobNature"></div>

            <div class="titles" style="margin-bottom: -16px;">职位描述</div>
            <div v-html="jobsData.intro"></div>
        ...
        <el-card>
          ...
          <el-row>
            <el-col :span="12" style="text-align: center">
              <router-link    :to="'/jobManage/jobsPublish?id=' +jobsData.id">
                <el-button type="primary" >修改职位</el-button>
              </router-link>
            </el-col>
            <el-col :span="12" style="text-align: center">
              <el-button type="primary" @click="closeJobsFun(jobsData.id)">关闭职位</el-button>
            </el-col>
          </el-row>
        </el-card>
...
import {
  addCompanyJobs, closeCompanyJobsData,
  editCompanyJobs,
  getCompanyJobDetail, getJobLogsData,
  listCompanyAddress,
  listJobTypeData, refreshCompanyJobsData
  ...
  methods: {
    ...
    getJobDetail(id) {
      getCompanyJobDetail(id).then(response => {
        ...
        console.log("ooo",this.jobsData.intro)
      });
    },
    getJobLogsList(id) {
    ...
    closeJobsFun(jobId){
      this.$modal.confirm('确认是否关闭该职位？').then(function() {
        return closeCompanyJobsData(jobId);
      }).then(() => {
        this.getJobLogsList(jobId);
        this.getJobDetail(jobId);
