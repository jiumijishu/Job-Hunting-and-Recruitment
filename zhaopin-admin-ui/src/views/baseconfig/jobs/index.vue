    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="80px">
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择状态" clearable>
          ...
          <el-option label="正常" value="02"/>
          <el-option label="关闭" value="00"/>
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="推荐" prop="referrerFlag">
        <el-select v-model="queryParams.referrerFlag" placeholder="请选择是否推荐" clearable>
          <el-option
      ...
      <el-form-item label="急聘" prop="helpWanted">
        <el-select v-model="queryParams.helpWanted" placeholder="请选择是否急聘" clearable>
          <el-option
            v-for="dict in dict.type.sys_yes_no"
            :key="dict.value"
            :label="dict.label"
...
<!--    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['baseconfig:jobs:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['baseconfig:jobs:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['baseconfig:jobs:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['baseconfig:jobs:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" border :data="jobsList">
      ...
      <el-table-column label="急聘" align="center" prop="helpWanted" >
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_yes_no" :value="scope.row.helpWanted"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['baseconfig:jobs:remove']"
          >{{scope.row.referrerFlag==='Y'?'取消推荐':'推荐'}}</el-button>

          <el-button
            size="mini"
            type="text"
            icon="el-icon-share"
            @click="handleHelpWanted(scope.row)"
            v-hasPermi="['baseconfig:jobs:remove']"
          >{{scope.row.helpWanted==='Y'?'取消急聘':'急聘'}}</el-button>
        </template>
...
<script>
import {listJobs, getJobs, delJobs, addJobs, updateJobs, listJobsAll} from "@/api/baseconfig/jobs";
...
export default {
  ...
  methods: {
    ...
    /** 急聘按钮操作 */
    handleHelpWanted(row) {
      let param={
        id:row.id,
        helpWanted:row.helpWanted==='Y'?'N':'Y'
      }
      this.$modal.confirm('是否确认该操作？').then(function() {
        return updateJobs(param);
      }).then(() => {
