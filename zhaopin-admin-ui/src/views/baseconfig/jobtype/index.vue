  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['baseconfig:jobtype:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" border :data="jobtypeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="职位简述" align="center" prop="introduction" />
      <el-table-column label="职位标签" align="center" prop="lables" />
      <el-table-column label="职位介绍" align="center" prop="details" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="650px" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="ID" prop="categoryId">
          <el-input v-model="form.categoryId" placeholder="请输入ID" />
        </el-form-item>
        <el-form-item label="类型名称" prop="categoryName">
          <el-input v-model="form.categoryName" placeholder="请输入类型名称" />
        </el-form-item>
        <el-form-item label="职位类型" prop="typeName">
          <el-input v-model="form.typeName" placeholder="请输入职位类型" />
        </el-form-item>
        <el-form-item label="职位简述" prop="introduction">
          <el-input v-model="form.introduction" type="textarea" placeholder="请输入内容" />
        </el-form-item>
        <el-form-item label="职位标签" prop="lables">
          <el-input v-model="form.lables" type="textarea" placeholder="请输入内容" />
        </el-form-item>
...
<script>
import { listJobtype, getJobtype, delJobtype, addJobtype, updateJobtype } from "@/api/baseconfig/jobtype";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listJobtype(this.queryParams).then(response => {
        this.jobtypeList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateJobtype(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addJobtype(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('baseconfig/jobtype/export', {
        ...this.queryParams
      }, `jobtype_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
